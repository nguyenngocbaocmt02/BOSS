...
from sde_lib import RectifiedFlow, EDM
...
def get_rectified_flow_loss_fn(sde, train, reduce_mean=True,  eps=1e-3, return_loss_each_samples=False):
  ...
  def loss_fn(model, batch):
    ...
    if sde.mode=="reflow":
        if sde.reflow_t_schedule=='t0': ### distill for t = 0 (k=1)
            ...
            try:
              t = t = torch.randint(0, int(sde.reflow_t_schedule), (batch.shape[0], ), device=batch.device) * (sde.T - eps) / int(sde.reflow_t_schedule) + eps
            except:
    ...
    if return_loss_each_samples:
      return loss, losses
    return loss

  return loss_fn

def get_redress_loss_fn(sde, train, reduce_mean=True, return_loss_each_samples=False):
  """Create a loss function for training with rectified flow.
  ...
  def loss_fn(model, batch):
    ...
    t0_expand = t0.view(-1, 1, 1, 1).repeat(1, batch.shape[1], batch.shape[2], batch.shape[3])
    t1_expand = t1.view(-1, 1, 1, 1).repeat(1, batch.shape[1], batch.shape[2], batch.shape[3])
    target = (batch - z0) / (t1_expand - t0_expand)
    model_fn = sde.wrap_model(model, train=train)
    score = model_fn(z0, t0)  ### Copy from models/utils.py 
    if sde.mode=="redress":
        ### we found LPIPS loss is the best for distillation when k=1; but good to have a try
        if sde.redress_loss=='l2':
            ### train new rectified flow with reflow or distillation with L2 loss
            losses = torch.square(score - target)
        elif sde.redress_loss=='lpips':
            losses = sde.lpips_model(z0 + score, batch)
        elif sde.redress_loss=='lpips+l2':
            lpips_losses = sde.lpips_model(z0 + score, batch).view(batch.shape[0], 1)
            l2_losses = torch.square(score - target).view(batch.shape[0], -1).mean(dim=1, keepdim=True)
            losses = lpips_losses + l2_losses
        else:
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False, return_loss_each_samples=False):
  ...
  """
  if sde.mode == "redress":
    loss_fn = get_redress_loss_fn(sde, train=train, reduce_mean=reduce_mean, return_loss_each_samples=return_loss_each_samples)
  else:
    if continuous:
      ...
      if isinstance(sde, RectifiedFlow):
        loss_fn = get_rectified_flow_loss_fn(sde, train, reduce_mean=reduce_mean, return_loss_each_samples=return_loss_each_samples)
      else:
